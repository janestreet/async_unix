(** A thread pool is a set of OCaml threads used to do work, where each piece of work is
    ...
    it. Work is done first-come-first-served by available threads in the pool. Any of the
    available threads in the pool could be used to do work submitted to the pool (except
    helper threads, see below).
    ...
    [max_num_threads], supplied to [create]. Thread-pool threads never die. They just get
    created up until [max_num_threads] is reached and then live forever, doing work. Each
    thread in the pool is in a loop, waiting for a piece of work, running the thunk, and
    then repeating. It may be that all the threads in the pool are not doing anything, but
    in this case, the threads still exist, and are simply blocked waiting for work.
    ...
    sense of [Linux_ext.setpriority]). Work added to the pool can optionally be given a
    priority, and the pool will set the priority of the thread that runs it for the
    duration of the work. Helper threads can also be given a priority, which will be used
    for all work run by the helper thread, unless the work has an overriding priority. The
    thread pool has a "default" priority that will be used for all work and helper threads
    that have no specified priority. The default priority is the priority in effect when
    [create] is called.

    Affinity, on the other hand, can only be specified when you create a pool. The default
    affinity is the affinity in effect when a new thread happens to be created (e.g. when
    you call [add_work]).
...
(** [create ?cpuset ~max_num_threads] returns a new thread pool. It is an error if
    [max_num_threads < 1].

    If [cpuset] is specified, then every thread will be affinitized to those CPU cores
    upon creation.

    If [cpuset] is not specified, then every thread will inherit the affinitization of the
    thread (typically the main thread) that created it. *)
val create
...
(** [cpu_affinity t] returns the CPU affinity that [t] was created with. All threads
    created by [t] will be created with this affinity. *)
...
(** [unfinished_work t] returns the number of jobs that have been submitted to [t] but
    haven't yet finished. *)
val unfinished_work : t -> int

(** [default_priority t] returns the priority that will be used for work performed by [t],
...
(** [add_work ?priority ?name t f] enqueues [f] to be done by some thread in the pool.
    ...
    [Linux_ext.pr_set_name]) to [name] and the priority of the thread will be set to
    [priority].
...
(** [has_unstarted_work t] returns [true] if [t] has work that it hasn't been assigned to
...
(** A helper thread is a thread with its own dedicated work queue. Work added for the
    helper thread is guaranteed to be run by that thread. The helper thread only runs work
    explicitly supplied to it. Helper threads count towards a thread pool's
    [max_num_threads]. *)
...
(** [create_helper_thread ?priority ?name t] takes an available thread from the thread
    ...
    and [priority], except for work that is added with an overriding priority or name. The
    thread remains a helper thread until [finished_with_helper_thread] is called, if ever. *)
...
(** [become_helper_thread ?priority ?name t] should be run from within work supplied to
    [add_work]. When [become_helper_thread] runs, it transitions the current thread into a
    helper thread.
...
module Stats : sig
  type t =
    ...
    ; unfinished_work : int
    (** [unfinished_work] is the current total number of jobs that have been added to the
        thread pool but not completed, including work added to helper threads via
...
val get_and_reset_stats_local : t -> Stats.t
