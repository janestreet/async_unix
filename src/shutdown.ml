...
let don't_finish_before ?(here = Stdlib.Lexing.dummy_pos) d =
  let bag_elt = at_shutdown_removable ~here normal_handlers (fun () -> d) in
  upon d (fun () -> Bag.remove normal_handlers bag_elt)
;;

module Private = struct
  let run_after_other_shutdown_handlers ?(here = Stdlib.Lexing.dummy_pos) f =
