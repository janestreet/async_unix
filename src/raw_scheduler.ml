...
let have_lock_do_cycle_default ?additional_cycle_time t =
...
let have_lock_do_cycle ?additional_cycle_time t =
...
let go ?raise_unhandled_exn ?(here = Stdlib.Lexing.dummy_pos) () =
...
module External = struct
  ...
  let run_one_cycle ?(here = Stdlib.Lexing.dummy_pos) t =
    set_task_id ();
    let active =
      match t.start_type with
