...
module type S = sig
  (** A file-descr-watcher is essentially a map from [File_descr.t] to
      [bool Read_write_pair.t], which defines the set of file descriptors being watched,
      and for each file descriptor, whether it is being watched for read, write, or both.
      If a file descriptor is not being watched for either, it is not in the map. *)
  ...
  (** [set] alters the map of file descriptors being watched. It will take effect on the
      next call to [thread_safe_check]. Calling [set fd] with
      [{ read = false, write = false }] removes [fd] from the map. [`Unsupported] can be
      returned for file descriptors that do not support polling. This usually (always?)
      means that they are always ready for read or write. *)
  val set : t -> File_descr.t -> bool Read_write_pair.t -> [ `Ok | `Unsupported ]

  (** [iter t ~f] iterates over every file descriptor in the map, apply [f] to it once for
  ...
  (** [post_check t check_result] calls the [handle_fd*] functions supplied to [create]:
      ...
      external world before we process all the reads. This will nicely batch together all
      the output based on the reads for the next writes.
