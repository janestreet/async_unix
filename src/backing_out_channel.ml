...
let of_output_char output_char : t =
  create ~output_char ~flush:Fn.id ~sexp:(fun () -> [%sexp ()])
    ~output_chars:(fun buf ~len -> for i = 0 to len - 1 do output_char buf.{i} done)
;;
...
(* Implement the polymorphic [write_gen_* ~blit_to_string] API here rather than in every
   ...
   very little cost in abstraction - it only gives up buffer managmement details. *)
